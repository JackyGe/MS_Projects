...
function purgeCompletedItems() {
    ...
    employeeList = clientContext.get_web().get_lists().getByTitle('New Employees In Seattle');
    var camlQuery = new SP.CamlQuery();
    camlQuery.set_viewXml(
          '<View><Query><Where><Eq>' +
            '<FieldRef Name=\'OrientationStage\'/><Value Type=\'Text\'>Completed</Value>' +
          '</Eq></Where></Query></View>');
...
function ensureOrientationScheduling() {
    ...
    camlQuery.set_viewXml(
        '<View><Query><Where><Eq>' +
            '<FieldRef Name=\'OrientationStage\'/><Value Type=\'Text\'>Not started</Value>' +
        '</Eq></Where></Query></View>');
...
function scheduleAsNeeded() {
    ...
    while (listItemEnumerator.moveNext()) {
        ...
        if (alreadyScheduled === false) {
            ...
            itemToCreate.set_item('EventDate', '2015-12-' + dayOfMonth + 'T21:00:00Z');
            itemToCreate.set_item('EndDate', '2015-12-' + dayOfMonth + 'T23:00:00Z');
            dayOfMonth++;
